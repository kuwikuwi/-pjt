...
export default {
  data() {
    return {
      ...
      currency_one: 'KRW',
      currency_two: 'USD',
      currency_JPN: 'JPY',  // Corrected typo in currency codes
      currency_TWD: 'TWD',  // Corrected typo in currency codes
      currency_CNY: 'CNY',  // Corrected typo in currency codes

      rate: "",
      amountOne: 1,
      amountTwo: 0,
      amountFor1000KRW: {},  // New property to store rates for 1000 KRW
    };
  },
  methods: {
    async fetchData() {
      try {
        const response = await fetch(
          `https://v6.exchangerate-api.com/v6/ce805e9ef25c087817b20a92/latest/${this.currency_one}`
        );
        const data = await response.json();
        this.data = data;
        this.rate = data.conversion_rates[this.currency_two];
        this.amountTwo = (this.amountOne * this.rate).toFixed(7);

        // Fetch rates for 1000 KRW to CNY and JPY
        this.fetchRatesFor1000KRW();
      } catch (error) {
        console.error('Error fetching exchange rates:', error);
      }
    },
    async fetchRatesFor1000KRW() {
      try {
        const response = await fetch(
          `https://v6.exchangerate-api.com/v6/ce805e9ef25c087817b20a92/latest/KRW`
        );
        const data = await response.json();
        this.amountFor1000KRW.CNY = (1000 * data.conversion_rates.CNY).toFixed(2)
        this.amountFor1000KRW.JPY = (1000 * data.conversion_rates.JPY).toFixed(2)
        this.amountFor1000KRW.USD = (1000 * data.conversion_rates.USD).toFixed(2)
        this.amountFor1000KRW.EUR = (1000 * data.conversion_rates.EUR).toFixed(2)
      } catch (error) {
        console.error('Error fetching rates for 1000 KRW:', error);
      }
    },
    switchValues() {
      [this.currency_one, this.currency_two] = [this.currency_two, this.currency_one];
      this.fetchData();
    },
  },
  mounted() {
...
<template>
  <h1 class="text">환율계산기</h1>
    <div class="container">
      <div class="container-one">
        <select v-model="currency_one" @change="fetchData" class="select">
        ...
        <input class="input" type="number" v-model="amountOne" @input="fetchData" />
      </div>
      <br>
      <div class="container-two">
        <button class="button" @click="switchValues()">변환</button>
        <h4 id="baseValue">
          1 {{ currency_one }} = {{ rate }} {{ currency_two }}
        </h4>
      </div>
      <div class="container-three">
        <select v-model="currency_two" @change="fetchData" class="select">
          <option value="AED"> AED - Emirati Dirham</option>
          <option value="AFN">AFN - Afghan Afghani</option>
          <option value="ALL">ALL - Albanian Lek</option>
          <option value="AMD">AMD - Armenian Dram</option>
        ...
        <input class="input" type="number" id="amount-two" placeholder="0" disabled v-model="amountTwo" />
      </div>
      <div class="container-four">
        <h4 id="lastlyUpdated">갱신 시간: {{data.time_last_update_utc}}</h4>
      </div>
    </div>

  <h1 class="text">인기 여행지 환율</h1>
  <p class="p-text">1000원 기준</p>
  <div class="container">
    <table class="table">
      <tr>
        <td><img src="../img/china.png" alt=""></td>
        <td><img src="../img/japan.PNG" alt=""></td>
        <td><img src="../img/usa.PNG" alt=""></td>
        <td><img src="../img/euro.PNG" alt=""></td>
      </tr>
  <tr>
    <td>위안 (CNY)</td>
    <td>엔 (JPY)</td>
  ...
  <tr>
    <td>{{ amountFor1000KRW.CNY }}¥</td>
    <td>{{ amountFor1000KRW.JPY }}¥</td>
    <td>{{ amountFor1000KRW.USD }}$</td>
    <td>{{ amountFor1000KRW.EUR }}€</td>
  </tr>
</table>
